class Standard
  ...
  def coil_cooling_dx_two_speed_standard_minimum_cop(coil_cooling_dx_two_speed, rename = false)
    ...
    if (database[0].keys.include?('equipment_type')) && !OpenstudioStandards::HVAC.coil_dx_heat_pump?(coil_cooling_dx_two_speed)
  ...
  def coil_cooling_dx_two_speed_apply_efficiency_and_curves(coil_cooling_dx_two_speed, sql_db_vars_map)
    ...
    # Additional search criteria
    if (coil_efficiency_data[0].keys.include?('equipment_type')) && !OpenstudioStandards::HVAC.coil_dx_heat_pump?(coil_cooling_dx_two_speed)
      search_criteria['equipment_type'] = 'Air Conditioners'
    end
