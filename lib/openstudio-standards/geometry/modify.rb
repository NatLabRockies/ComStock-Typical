module OpenstudioStandards
  ...
  module Geometry
    ...
    # Match the surfaces of every space in the model against every other space, so that
    # shared surfaces get each other as their adjacent surface.
    #
    # @param model [OpenStudio::Model::Model] OpenStudio Model object
    # @return [OpenStudio::Model::Model] the model object
    def self.model_match_surfaces(model)
    ...
    # Rotate the model geometry about the z axis. This turns the geometry itself, unlike
    # model_set_building_north_axis, which only changes the building's north axis property.
    #
    # @param model [OpenStudio::Model::Model] OpenStudio Model object
    # @param degrees [Float] rotation in degrees
    # @return [OpenStudio::Model::Model] the model object
    def self.model_rotate(model, degrees)
      transformation = OpenStudio::Transformation.rotation(OpenStudio::Vector3d.new(0, 0, 1),
                                                          OpenStudio.degToRad(degrees))
      model.getPlanarSurfaceGroups.each { |planar_surface| planar_surface.changeTransformation(transformation) }

      return model
    end

    # Set the outside boundary condition on surfaces and on their matched surfaces.
    # Adiabatic surfaces cannot carry subsurfaces in EnergyPlus, so subsurfaces are removed
    # and a defaulted construction is hard-assigned before the condition is changed.
    #
    # @param surfaces [Array<OpenStudio::Model::Surface>] Array of OpenStudio Surface objects
    # @param boundary_condition [String] outside boundary condition, e.g. 'Ground', 'Adiabatic'
    # @return [Array<OpenStudio::Model::Surface>] the surfaces
    def self.surfaces_set_boundary_condition(surfaces, boundary_condition)
      unless OpenStudio::Model::Surface.validOutsideBoundaryConditionValues.include?(boundary_condition)
        OpenStudio.logFree(OpenStudio::Error, 'openstudio.standards.Geometry',
                           "'#{boundary_condition}' is not a valid outside boundary condition. Valid values are: #{OpenStudio::Model::Surface.validOutsideBoundaryConditionValues.join(', ')}.")
        return surfaces
      end
      ...
      surfaces.each do |surface|
        if boundary_condition == 'Adiabatic'
          ...
          # adiabatic surfaces do not hold a defaulted construction, so hard-assign it
          surface.setConstruction(surface.construction.get) if surface.isConstructionDefaulted && surface.construction.is_initialized
        end

        surface.setOutsideBoundaryCondition(boundary_condition)
        adjacent_surface = surface.adjacentSurface
        adjacent_surface.get.setOutsideBoundaryCondition(boundary_condition) unless adjacent_surface.empty?
      end
