...
class TestNistInfiltration < Minitest::Test
  def setup
  ...
  def test_model_infer_nist_building_type
    ...
    model = std.safe_load_model("#{File.dirname(__FILE__)}/../../models/geometry/ASHRAESecondarySchool.osm")
    building_type = @infiltration.model_infer_nist_building_type(model)
    assert('SecondarySchool', building_type)

    model = std.safe_load_model("#{File.dirname(__FILE__)}/../../models/geometry/ASHRAELargeHotel.osm")
    building_type = @infiltration.model_infer_nist_building_type(model)
    ...
    model = std.safe_load_model("#{File.dirname(__FILE__)}/../../models/geometry/DEER_RtL.osm")
    building_type = @infiltration.model_infer_nist_building_type(model)
    assert('RetailStripmall', building_type)

    model = std.safe_load_model("#{File.dirname(__FILE__)}/../../models/geometry/DEER_SUn.osm")
    building_type = @infiltration.model_infer_nist_building_type(model)
    assert('RetailStripmall', building_type)

    model = std.safe_load_model("#{File.dirname(__FILE__)}/../../models/geometry/DEER_MFm.osm")
    building_type = @infiltration.model_infer_nist_building_type(model)
