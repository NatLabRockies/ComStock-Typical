...
class TestApplyHVACEfficiencyStandard < Minitest::Test
  def test_pthp_90_1_2019
    ...
    model.getCoilHeatingDXSingleSpeeds.each do |coil|
      # find ac properties
      search_criteria = std.coil_dx_find_search_criteria(coil)
  ...
  def test_vrf_doe_ref_pre_1980
    ...
    model = std.safe_load_model("#{File.dirname(__FILE__)}/../models/bldg_20.osm")
    OpenstudioStandards::Weather.model_set_building_location(model, climate_zone: 'ASHRAE 169-2013-4A')
  ...
  def test_unitary_ac_eff_lookups
      ...
      # PTAC
      std = Standard.build('90.1-2019')
      cop = std.coil_cooling_dx_single_speed_standard_minimum_cop(coil_cooling_dx_single_speed, false, true)
      ...
      cop = std.coil_cooling_dx_single_speed_standard_minimum_cop(coil_cooling_dx_single_speed, false, false)
